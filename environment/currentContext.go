...
func (c *currentContext) validatecurrentContext() error {
	...
	// configuration folder
	if err := utils.FileExist(c.ConfigFolder); os.IsNotExist(err) {
		return err
	}
	// configuration file
	if err := utils.FileExist(c.ConfigFolder + string(os.PathSeparator) + c.SetupFile); os.IsNotExist(err) {
		return err
	}
	// execute the indicated check
	// at this point we couldn't validate if the check exists because chekcs haven't already been loaded
	if c.ExecuteCheck != "" && c.ExecuteCheckGroup != "" {
		return errors.New("You should decide between execute a check or a checkgroup")
	}
	// loglevel definition
	if c.Loglevel < 0 || c.Loglevel > 3 {
		return errors.New("Undefined loglevel mode")
	}
	// execution mode
	if c.ExecutionMode != "standalone" && c.ExecutionMode != "cluster" {
		return errors.New("The execution mode chosen is unknown by Verdmell")
	}
	// nodes only take sense in cluster mode
	if c.ExecutionMode == "standalone" && len(c.Nodes) > 0 {
		return errors.New("The node only have sense on cluster's execution mode")
	}
	// host to anchor to server mode
	if err := utils.IsLocalIPAddress(c.Host); err != nil {
		return err
	}
	// port to anchor to server mode
	if c.Port < 0 || c.Port > 65535 {
		return errors.New("Is not possible to use a port out of 0..65535 range")
	}
...
//
// String method transforms the currentContext to string
