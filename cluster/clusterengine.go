	...
	if modify {
		...
		if err, clusternode := cluster.GetNode(env.Whoami()); err != nil {
			...
			if err, message := NewClusterMessage(env.Setup.Hostname, clusternode.GetTimestamp(), cluster); err == nil {
				...
				if messageBytes, err := utils.InterfaceToBytes(message); err == nil {
...
//
// GetCluster method gets whole information from cluster
func (c *ClusterEngine) GetClusterInfo() ([]byte, error) {
	env.Output.WriteChDebug("(ClusterEngine::GetClusterData)")
	var cluster *Cluster

	if cluster = c.GetCluster(); cluster == nil {
		return nil, errors.New("(ClusterEngine::GetClusterData) Cluster object has not been initialized")
	}
...
//
// GetClusterNodes method gets all nodes from cluster
func (c *ClusterEngine) GetClusterNodes() ([]byte, error) {
	env.Output.WriteChDebug("(ClusterEngine::GetClusterNodes)")
	var cluster *Cluster

	if cluster = c.GetCluster(); cluster == nil {
		return nil, errors.New("(ClusterEngine::GetClusterNodes) Cluster object has not been initialized")
	}
...
//
// GetClusterServices method gets all nodes from cluster
func (c *ClusterEngine) GetClusterServices() ([]byte, error) {
	env.Output.WriteChDebug("(ClusterEngine::GetClusterServices)")
	var cluster *Cluster

	if cluster = c.GetCluster(); cluster == nil {
		return nil, errors.New("(ClusterEngine::GetClusterServices) Cluster object has not been initialized")
	}
...
//
// String method converts a ClusterEngine object to string
func (c *ClusterEngine) String() string {
	var str string
	var err error

	str, err = utils.ObjectToJSONString(c.GetCluster())
	if err != nil {
		return err.Error()
	}
